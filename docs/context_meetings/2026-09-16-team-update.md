...
> **Note:** These videos use ROS 2 Foxy and Gazebo Classic. The ideas are the same for us, but some commands are different. We use ROS 2 Humble with the newer Gazebo (`gz sim`, `ros_gz`), so follow our repo for the actual setup commands.
